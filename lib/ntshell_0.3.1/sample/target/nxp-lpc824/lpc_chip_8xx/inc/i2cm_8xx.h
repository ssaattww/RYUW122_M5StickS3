...
extern "C"
{
   ...
   * to get a 10 bit I2C address. Data  (8 bits): A plain data byte. Sometimes,
   * I write DataLow, DataHigh for 16 bit data.
...
#define I2CM_STATUS_NAK_ADR                                                    \
  0x02 /*!< No acknowledgement received from slave during address phase. */
#define I2CM_STATUS_BUS_ERROR 0x03 /*!< I2C bus error */
#define I2CM_STATUS_NAK_DAT                                                    \
  0x04 /*!< No acknowledgement received from slave during address phase. */
...
#define I2CM_STATUS_BUSY 0xFF    /*!< I2C transmistter is busy. */
  ...
  typedef struct
  {
    ...
    uint8_t
        *rxBuff; /*!< Pointer memory where bytes received from I2C be stored */
    uint16_t txSz;     /*!< Number of bytes in transmit array,
                                                       if 0 only receive transfer
                          will be carried on */
    uint16_t rxSz;     /*!< Number of bytes to received,
                                                       if 0 only transmission we be
                          carried on */
   ...
   * @param	pI2C	: Pointer to selected I2C peripheral
   * @param clrStatus : Status bit to clear, ORed Value of I2C_STAT_MSTRARBLOSS
   * and I2C_STAT_MSTSTSTPERR
   * @return	Nothing
   ...
   * @param	pI2C	: Pointer to selected I2C peripheral
   * @return	Nothing
   * @note	This function sets the controller to transmit START condition
   * when the bus becomes free. This should be called only when master is
   * pending. The function writes a complete value to Master Control register,
   * ORing is not advised.
   */
   ...
   *				This should be called only when master is
   * pending. The function writes a complete value to Master Control register,
   * ORing is not advised.
   */
   ...
   * @param	pI2C	: Pointer to selected I2C peripheral
   * @return	Nothing
   * @note	This function sets the master controller to continue
   * transmission. This should be called only when master is pending. The
   * function writes a complete value to Master Control register, ORing is not
   * advised.
   */
   ...
   * from slave be stored and the number of data to get from slave
   * respectilvely. Following types of transfers are possible:
   ...
   *
   * - Read-Write transfer: When @a rxSz and @ txSz members of @a xfer are
   * non-zero.
   *
