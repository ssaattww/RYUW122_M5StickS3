...
/**
 * @brief UART ROM driver - UART errors in UART configuration used in uart_init
 * function
 */
...
#define ALL_ERR_EN                                                             \
  (OVERRUN_ERR_EN | UNDERRUN_ERR_EN | FRAME_ERR_EN | PARITY_ERR_EN |           \
   RXNOISE_ERR_EN)
...
/*!< 0x01: uart_put_line: transfer stopped after reaching \0 and CRLF is sent
 * out after that */
#define TX_MODE_SZERO_SEND_CRLF (0x01)
#define RX_MODE_CRLF_RECVD (0x01)
/*!< 0x02: uart_get_line: stop transfer when LF are received */
/*!< 0x02: uart_put_line: transfer stopped after reaching \0. And LF is sent out
 * after that */
...
#define DRIVER_MODE_DMA (0x02)       /*!< DMA mode */
   ...
   */
  typedef void (*UART_DMA_REQ_T)(uint32_t src_adr, uint32_t dst_adr,
                                 uint32_t size);
  ...
  typedef struct
  {
    ...
    /*!<  bit1:0  Data Length: 00: 7 bits length, 01: 8 bits length, others:
     * reserved */
  ...
  typedef struct
  {
    ...
    /*!<   0x00: uart_put_line: stop transfer when the buffer is empty */
    /*!<   0x01: uart_get_line: stop transfer when CRLF are received */
    /*!<   0x01: uart_put_line: transfer stopped after reaching \0 and CRLF is
     * sent out after that */
    /*!<   0x02: uart_get_line: stop transfer when LF are received */
    /*!<   0x02: uart_put_line: transfer stopped after reaching \0 and LF is
     * sent out after that */
    /*!<   0x03: uart_get_line: RESERVED */
    /*!<   0x03: uart_put_line: transfer stopped after reaching \0 */
    uint16_t driver_mode; /*!< Driver mode */
    /*!<  0x00: Polling mode, function blocked until transfer completes */
    /*!<  0x01: Interrupt mode, function immediately returns, callback invoked
     * when transfer completes */
    /*!<  0x02: DMA mode, in case DMA block is available, DMA req function is
     * called for UART DMA channel setup, then callback function indicate that
     * transfer completes */
    UART_CALLBK_T callback_func_pt; /*!< callback function pointer */
    UART_DMA_REQ_T dma_req_func_pt; /*!< UART DMA channel setup function
                                       pointer, not applicable on LPC8xx */
  } UART_PARAM_T;
  ...
  typedef struct UARTD_API
  {
    ...
    uint32_t (*uart_get_mem_size)(
        void); /*!< Get the memory size needed by one Min UART instance */
    UART_HANDLE_T *(*uart_setup)(
        uint32_t base_addr,
        uint8_t *ram); /*!< Setup Min UART instance with provided memory and
                          return the handle to this instance */
    uint32_t (*uart_init)(
        UART_HANDLE_T *handle,
        UART_CONFIG_T *set); /*!< Setup baud rate and operation mode for uart,
                                then enable uart */

    /*!< UART polling functions block until completed */
    uint8_t (*uart_get_char)(
        UART_HANDLE_T
            *handle); /*!< Receive one Char from uart. This functions is only
                         returned after Char is received. In case Echo is
                         enabled, the received data is sent out immediately */
    void (*uart_put_char)(
        UART_HANDLE_T *handle,
        uint8_t data); /*!< Send one Char through uart. This function is only
                          returned after data is sent */
    uint32_t (*uart_get_line)(
        UART_HANDLE_T *handle,
        UART_PARAM_T *param); /*!< Receive multiple bytes from UART */
    uint32_t (*uart_put_line)(
        UART_HANDLE_T *handle,
        UART_PARAM_T
            *param); /*!< Send string (end with \0) or raw data through UART */

    /*!< UART interrupt functions return immediately and callback when completed
     */
    void (*uart_isr)(
        UART_HANDLE_T
            *handle); /*!< UART interrupt service routine. To use this routine,
                         the corresponding USART interrupt must be enabled. This
                         function is invoked by the user ISR */
  } UARTD_API_T;
