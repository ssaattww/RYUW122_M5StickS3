...
extern "C"
{
  ...
  {                     /*!< (@ 0xA0004000) PIN_INT Structure */
    ...
    __IO uint32_t
        IENR; /*!< (@ 0xA0004004) Pin Interrupt Enable (Rising) register */
    __IO uint32_t
        SIENR; /*!< (@ 0xA0004008) Set Pin Interrupt Enable (Rising) register */
    __IO uint32_t CIENR; /*!< (@ 0xA000400C) Clear Pin Interrupt Enable (Rising)
                            register */
    __IO uint32_t IENF;  /*!< (@ 0xA0004010) Pin Interrupt Enable Falling Edge /
                            Active Level register */
    __IO uint32_t SIENF; /*!< (@ 0xA0004014) Set Pin Interrupt Enable Falling
                            Edge / Active Level register */
    __IO uint32_t CIENF; /*!< (@ 0xA0004018) Clear Pin Interrupt Enable Falling
                            Edge / Active Level address */
    __IO uint32_t
        RISE; /*!< (@ 0xA000401C) Pin Interrupt Rising Edge register */
    __IO uint32_t
        FALL; /*!< (@ 0xA0004020) Pin Interrupt Falling Edge register */
    __IO uint32_t IST;    /*!< (@ 0xA0004024) Pin Interrupt Status register */
    __IO uint32_t PMCTRL; /*!< (@ 0xA0004028) GPIO pattern match interrupt
                             control register          */
    __IO uint32_t PMSRC;  /*!< (@ 0xA000402C) GPIO pattern match interrupt
                             bit-slice source register */
    __IO uint32_t PMCFG;  /*!< (@ 0xA0004030) GPIO pattern match interrupt bit
                             slice configuration register */
  } LPC_PIN_INT_T;
...
#define PININTCH(ch) (1 << (ch))
   ...
   * @return	Nothing
   * @note	This function should be used after the Chip_GPIO_Init()
   * function.
   */
  ...
  /**
   * @brief	Return current PININT rising edge or high level interrupt enable
   * state
   * @param	pPININT	: The base address of Pin interrupt block
   * @return	A bifield containing the high edge/level interrupt enables for
   * each interrupt. Bit 0 = PININT0, 1 = PININT1, etc. For each bit, a 0 means
   * the high edge/level interrupt is disabled, while a 1 means it's enabled.
   */
  ...
  /**
   * @brief	Return current PININT falling edge or low level interrupt enable
   * state
   * @param	pPININT	: The base address of Pin interrupt block
   * @return	A bifield containing the low edge/level interrupt enables for
   * each interrupt. Bit 0 = PININT0, 1 = PININT1, etc. For each bit, a 0 means
   * the low edge/level interrupt is disabled, while a 1 means it's enabled.
   */
  ...
  /**
   * @brief	Return pin states that have a detected latched high edge (RISE)
   * state
   * @param	pPININT	: The base address of Pin interrupt block
  ...
  /**
   * @brief	Return pin states that have a detected latched falling edge
   * (FALL) state
   * @param	pPININT	: The base address of Pin interrupt block
   * @return	PININT states (bit n = high) with a latched rise state detected
   */
   ...
   * @param	pPININT	: The base address of Pin interrupt block
   * @return	Interrupt status (bit n for PININTn = high means interrupt ie
   * pending)
   */
   ...
   * @return	Nothing
   */
  void Chip_PININT_SetPatternMatchSrc(LPC_PIN_INT_T *pPININT, uint8_t chan,
                                      Chip_PININT_BITSLICE_T slice);

  /**
   * @brief	Configure the pattern matcch in Pin interrupt block
   * @param	pPININT		: The base address of Pin interrupt block
   * @param	slice		: PININT slice number
   * @param	slice_cfg	: PININT slice configuration value (enum
   * Chip_PININT_BITSLICE_CFG_T)
   * @param	end_point	: If true, current slice is final component
   * @return	Nothing
   */
  void Chip_PININT_SetPatternMatchConfig(LPC_PIN_INT_T *pPININT,
                                         Chip_PININT_BITSLICE_T slice,
                                         Chip_PININT_BITSLICE_CFG_T slice_cfg,
                                         bool end_point);
  ...
  {
    pPININT->PMCTRL =
        PININT_PMCTRL_PMATCH_SEL | (pPININT->PMCTRL & ~PININT_PMCTRL_RESERVED);
  }
  ...
  {
    pPININT->PMCTRL =
        PININT_PMCTRL_RXEV_ENA | (pPININT->PMCTRL & ~PININT_PMCTRL_RESERVED);
  }
