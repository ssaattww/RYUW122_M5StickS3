...
extern "C"
{
  ...
  {
    __IO uint32_t
        DMA_INMUX_INMUX[2];      /*!< DMA Trigger Input 20 & 21 PINMUX 0-1 */
  ...
  {
    SCT_INP_IN0,
    /*!< SCT0_IN0 selected by Pin Matrix */ /* FIXME: UM hints about changes */
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_INMUX_SetDMAOTrig(LPC_INMUX_T *pINMUX,
                                            DMA_INMUX_T imux, DMA_CHID_T ch)
  {
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_INMUX_SetSCTInMux(LPC_INMUX_T *pINMUX,
                                            SCT_INMUX_T isct, SCT_INP_T trig)
  {
   ...
   */
  typedef struct
  { /*!< DMA trigger pin muxing register structure */
    __IO uint32_t
        DMA_ITRIG_INMUX[MAX_DMA_CHANNEL]; /*!< Trigger input select register for
                                             DMA channels */
  } LPC_DMATRIGMUX_T;
  ...
  /**
   * @brief	Select a trigger source for a DMA channel
   * @param	pDMATRIG	: The base of DMA trigger setup block on the
   * chip
   * @param	ch			: DMA channel ID
   * @param	trig		: Trigger source for the DMA channel
   * @return	Nothing
   * @note	A DMA trigger source only needs to be setup when the DMA is
   * setup for hardware trigger mode (when Chip_DMA_SetupChannelConfig() is
   *			called with DMA_CFG_HWTRIGEN as OR'ed option).
   */
  STATIC INLINE void Chip_DMATRIGMUX_SetInputTrig(LPC_DMATRIGMUX_T *pDMATRIG,
                                                  DMA_CHID_T ch,
                                                  DMA_TRIGSRC_T trig)
  {
