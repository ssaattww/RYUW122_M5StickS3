...
#define UART_CFG_OETA                                                          \
  (0x01 << 18) /*!< Output Enable Turnaround time for RS485 */
...
#define UART_CFG_RESERVED                                                      \
  ((1 << 1) | (1 << 7) | (1 << 8) | (1 << 10) | (1 << 13) | (3 << 16) |        \
   (0xffu << 24))
#else
#define UART_CFG_RESERVED                                                      \
...
#define UART_INTEN_RESERVED                                                    \
  ((1 << 1) | (1 << 4) | (1 << 7) | (3 << 9) | (0xfffeu << 16))
#define UART_INTSTAT_RESERVED                                                  \
  ((1 << 1) | (1 << 4) | (1 << 7) | (3 << 9) | (0xfffeu << 16))
#else
#define UART_INTEN_RESERVED                                                    \
...
#define UART_INTSTAT_RESERVED                                                  \
   ...
   * The FIFO status should be read first prior to using this function
   */
   ...
   * @return	Nothing
   * @note	Use an OR'ed value of UART_INTEN_* definitions with this
   * function to enable specific UART interrupts.
   */
   ...
   * @return	Nothing
   * @note	Use an OR'ed value of UART_INTEN_* definitions with this
   * function to disable specific UART interrupts.
   */
   ...
   * @note	Use an OR'ed value of UART_INTEN_* definitions with this
   * function to determine which interrupts are enabled. You can check for
   * multiple enabled bits if needed.
   */
   ...
   * @param	pUART	: Pointer to selected pUART peripheral
   * @param	config	: UART configuration, OR'ed values of select UART_CFG_*
   * defines
   * @return	Nothing
   * @note	Select OR'ed config options for the UART from the
   * UART_CFG_PARITY_*, UART_CFG_STOPLEN_*, and UART_CFG_DATALEN_* definitions.
   * For example, a configuration of 8 data bits, 1 stop bit, and even (enabled)
   * parity would be (UART_CFG_DATALEN_8 | UART_CFG_STOPLEN_1 |
   * UART_CFG_PARITY_EVEN). Will not alter other bits in the CFG register.
   */
  STATIC INLINE void Chip_UART_ConfigData(LPC_USART_T *pUART, uint32_t config)
  {
    ...
    reg = pUART->CFG &
          ~((0x3 << 2) | (0x3 << 4) | (0x1 << 6) | UART_CFG_RESERVED);
   ...
   *			all the data is in the FIFO or the FIFO is full. This
   * function will not block in the FIFO is full. The actual number of bytes
   *			placed into the FIFO is returned. This function ignores
   * errors.
   */
   ...
   *			all the data has been read or the passed buffer is
   * completely full. This function will not block. This function ignores
   * errors.
   */
   ...
   *			FIFO. This function will block until the last bytes are
   * in the FIFO.
   */
  int Chip_UART_SendBlocking(LPC_USART_T *pUART, const void *data,
                             int numBytes);
   ...
   *			buffer is completely full. The function will block until
   * full. This function ignores errors.
   */
   ...
   *			transfer. If the number of bytes returned is less than
   * the number of bytes to send, the ring buffer is considered full.
   */
  uint32_t Chip_UART_SendRB(LPC_USART_T *pUART, RINGBUFF_T *pRB,
   ...
   */
  int Chip_UART_ReadRB(LPC_USART_T *pUART, RINGBUFF_T *pRB, void *data,
                       int bytes);
   ...
   */
  void Chip_UART_IRQRBHandler(LPC_USART_T *pUART, RINGBUFF_T *pRXRB,
                              RINGBUFF_T *pTXRB);
