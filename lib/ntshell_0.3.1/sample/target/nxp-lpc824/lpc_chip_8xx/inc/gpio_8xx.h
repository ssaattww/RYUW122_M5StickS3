...
extern "C"
{
  ...
  typedef struct
  {                          /*!< GPIO_PORT Structure */
    __IO uint8_t B[128][32]; /*!< Offset 0x0000: Byte pin registers ports 0 to
                                n; pins PIOn_0 to PIOn_31 */
    __IO uint32_t
        W[32][32];         /*!< Offset 0x1000: Word pin registers port 0 to n */
    ...
    __IO uint32_t SET[32];   /*!< Offset 0x2200: Write: Set register for port n
                                Read: output bits for port n */
   ...
   * @return	Nothing
   * @note	It is recommended to use the Chip_GPIO_SetPinState() function
   * instead.
   */
  STATIC INLINE void Chip_GPIO_WritePortBit(LPC_GPIO_T *pGPIO, uint32_t port,
                                            uint8_t pin, bool setting)
  {
   ...
   */
  STATIC INLINE void Chip_GPIO_SetPinState(LPC_GPIO_T *pGPIO, uint8_t port,
                                           uint8_t pin, bool setting)
  {
    pGPIO->B[port][pin] = setting;
  }
   ...
   * instead.
   */
  STATIC INLINE bool Chip_GPIO_ReadPortBit(LPC_GPIO_T *pGPIO, uint32_t port,
                                           uint8_t pin)
  {
   ...
   */
  STATIC INLINE bool Chip_GPIO_GetPinState(LPC_GPIO_T *pGPIO, uint8_t port,
                                           uint8_t pin)
  {
    return (bool)pGPIO->B[port][pin];
  }
   ...
   */
  STATIC INLINE void Chip_GPIO_WriteDirBit(LPC_GPIO_T *pGPIO, uint32_t port,
                                           uint8_t pin, bool setting)
  {
    if ( setting )
    {
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_GPIO_SetPinDIROutput(LPC_GPIO_T *pGPIO, uint8_t port,
                                               uint8_t pin)
  {
#ifdef CHIP_LPC82X
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_GPIO_SetPinDIRInput(LPC_GPIO_T *pGPIO, uint8_t port,
                                              uint8_t pin)
  {
#ifdef CHIP_LPC82X
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_GPIO_TogglePinDIR(LPC_GPIO_T *pGPIO, uint8_t port,
                                            uint8_t pin)
  {
#ifdef CHIP_LPC82X
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_GPIO_SetPinDIR(LPC_GPIO_T *pGPIO, uint8_t port,
                                         uint8_t pin, bool output)
  {
    if ( output )
    {
   ...
   * @note	It is recommended to use the Chip_GPIO_GetPinDIR() function
   * instead.
   */
  STATIC INLINE bool Chip_GPIO_ReadDirBit(LPC_GPIO_T *pGPIO, uint32_t port,
                                          uint8_t bit)
  {
   ...
   * @return	true if the GPIO is an output, false if input
   */
  STATIC INLINE bool Chip_GPIO_GetPinDIR(LPC_GPIO_T *pGPIO, uint8_t port,
                                         uint8_t pin)
  {
   ...
   * @param	out			: Direction value, 0 = input, !0 =
   * output
   * @return	None
   * @note	Bits set to '0' are not altered. Obsolete function use the
   * Chip_GPIO_SetPortDIR() function instead.
   */
  STATIC INLINE void Chip_GPIO_SetDir(LPC_GPIO_T *pGPIO, uint8_t portNum,
                                      uint32_t bitValue, uint8_t out)
  {
    if ( out )
    {
   ...
   * @param	pinMask	: GPIO pin mask to set direction on as output (bits 0..b
   * for pins 0..n)
   * @return	Nothing
   * @note	Sets multiple GPIO pins to the output direction, each bit's
   * position that is high sets the corresponding pin number for that bit to an
   * output.
   */
  STATIC INLINE void Chip_GPIO_SetPortDIROutput(LPC_GPIO_T *pGPIO, uint8_t port,
                                                uint8_t pinMask)
  {
#ifdef CHIP_LPC82X
   ...
   * @param	pGPIO	: The base of GPIO peripheral on the chip
   * @param	port	: port Number (supports port 0 only)
   * @param	pinMask	: GPIO pin mask to set direction on as input (bits 0..b
   * for pins 0..n)
   * @return	Nothing
   * @note	Sets multiple GPIO pins to the input direction, each bit's
   * position that is high sets the corresponding pin number for that bit to an
   * input.
   */
  STATIC INLINE void Chip_GPIO_SetPortDIRInput(LPC_GPIO_T *pGPIO, uint8_t port,
                                               uint8_t pinMask)
  {
#ifdef CHIP_LPC82X
   ...
   * @param	pGPIO	: The base of GPIO peripheral on the chip
   * @param	port	: port Number (supports port 0 only)
   * @param	pinMask	: GPIO pin mask Toggle direction (bits 0..n for pins
   * 0..n)
   * @return	Nothing
   * @note	Toggles multiple GPIO pin's direction, each bit's position that
   * is high toggles direction of the corresponding pin number.
   */
  STATIC INLINE void Chip_GPIO_TogglePortDIR(LPC_GPIO_T *pGPIO, uint8_t port,
                                             uint8_t pinMask)
  {
#ifdef CHIP_LPC82X
  ...
  /**
   * @brief	Set GPIO direction for a all selected GPIO pins to an input or
   * output
   * @param	pGPIO	: The base of GPIO peripheral on the chip
   * @param	port	: port Number (supports port 0 only)
   * @param	pinMask	: GPIO pin mask to set direction on (bits 0..b for pins
   * 0..n)
   * @param	outSet	: Direction value, false = set as inputs, true = set as
   * outputs
   * @return	Nothing
   * @note	Sets multiple GPIO pins to the input direction, each bit's
   * position that is high sets the corresponding pin number for that bit to an
   * input.
   */
  STATIC INLINE void Chip_GPIO_SetPortDIR(LPC_GPIO_T *pGPIO, uint8_t port,
                                          uint8_t pinMask, bool outSet)
  {
    if ( outSet )
    {
  ...
  /**
   * @brief	Set GPIO direction for set of pins as output and a set of pins
   * as input
   * @param	pGPIO	: The base of GPIO peripheral on the chip
   * @param	port	: port Number (supports port 0 only)
   * @param	pinMask	: GPIO pin mask to set direction on (bits 0..b for pins
   * 0..n)
   * @param	outMask	: GPIO IN/OUT mask to set pin n as output set bit-n (see
   * note for more info)
   * @return	Nothing
   * @note	Sets multiple GPIO pins to the input or output, to change the
   * direction of PIN0_1, PIN_05, PIN0_10 and PIN0_18, set the @a pinMask to ((1
   * << 1) | (1 << 5) | (1 << 10) | (1 << 18)) to set PIN0_5 and PIN0_18 to
   * output and PIN0_1 and PIN0_10 as input set @a outMask to
   * ((1 << 5) | (1 << 18)).
   */
  STATIC INLINE void Chip_GPIO_SetPortDIRMask(LPC_GPIO_T *pGPIO, uint8_t port,
                                              uint8_t pinMask, bool outMask)
  {
   ...
   * @note	For pins 0..n, a high state in a bit corresponds to an output
   * state for the same pin, while a low  state corresponds to an input state.
   */
   ...
   * @param	pGPIO	: The base of GPIO peripheral on the chip
   * @param	port	: port Number (supports port 0 only)
   * @param	mask	: Mask value for read and write (only low bits are
   * enabled)
   * @return	Nothing
   ...
   */
  STATIC INLINE void Chip_GPIO_SetPortMask(LPC_GPIO_T *pGPIO, uint8_t port,
                                           uint32_t mask)
  {
   ...
   * @note	A high bit in the return value indicates that that GPIO pin for
   * the port cannot be set using the masked write function.
   */
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_GPIO_SetPortValue(LPC_GPIO_T *pGPIO, uint8_t port,
                                            uint32_t value)
  {
   ...
   */
  STATIC INLINE void Chip_GPIO_SetValue(LPC_GPIO_T *pGPIO, uint8_t portNum,
                                        uint32_t bitValue)
  {
   ...
   */
  STATIC INLINE void Chip_GPIO_SetPortOutHigh(LPC_GPIO_T *pGPIO, uint8_t port,
                                              uint32_t pins)
  {
    pGPIO->SET[port] = pins;
  }
   ...
   * applies to ports configured as an output.
   */
  STATIC INLINE void Chip_GPIO_SetPinOutHigh(LPC_GPIO_T *pGPIO, uint8_t port,
                                             uint8_t pin)
  {
   ...
   */
  STATIC INLINE void Chip_GPIO_ClearValue(LPC_GPIO_T *pGPIO, uint8_t portNum,
                                          uint32_t bitValue)
  {
   ...
   * applies to ports configured as an output.
   */
  STATIC INLINE void Chip_GPIO_SetPortOutLow(LPC_GPIO_T *pGPIO, uint8_t port,
                                             uint32_t pins)
  {
    pGPIO->CLR[port] = pins;
  }
   ...
   * applies to ports configured as an output.
   */
  STATIC INLINE void Chip_GPIO_SetPinOutLow(LPC_GPIO_T *pGPIO, uint8_t port,
                                            uint8_t pin)
  {
   ...
   * applies to ports configured as an output.
   */
  STATIC INLINE void Chip_GPIO_SetPortToggle(LPC_GPIO_T *pGPIO, uint8_t port,
                                             uint32_t pins)
  {
    pGPIO->NOT[port] = pins;
  }
   ...
   * @note	The current states of the bits for the port are read, regardless
   * of whether the GPIO port bits are input or output. It is recommended to use
   * the Chip_GPIO_GetPortValue() function instead.
   */
