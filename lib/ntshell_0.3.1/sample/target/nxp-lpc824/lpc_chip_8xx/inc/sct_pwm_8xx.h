...
extern "C"
{
   ...
   * <a
   * href="http://www.lpcware.com/content/faq/how-use-sct-standard-pwm-using-lpcopen">
   * www.lpcware.com</a>
   *
   ...
   */
  STATIC INLINE void Chip_SCTPWM_SetDutyCycle(LPC_SCT_T *pSCT, uint8_t index,
                                              uint32_t ticks)
  {
   ...
   * @return	None
   */
  STATIC INLINE void Chip_SCTPWM_Init(LPC_SCT_T *pSCT) { Chip_SCT_Init(pSCT); }
   ...
   * 			configuration is completed. Do not call
   * Chip_SCTPWM_SetRate() or Chip_SCTPWM_SetOutPin() after the SCT/PWM is
   * started. Use Chip_SCTPWM_Stop() to stop the SCT/PWM before reconfiguring,
   * 			Chip_SCTPWM_SetDutyCycle() can be called when the
   * SCT/PWM is running to change the DutyCycle.
   */
