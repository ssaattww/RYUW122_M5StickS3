...
#define SPI_CFG_CPHA_FIRST                                                     \
  (0 << 4) /** Capture data on the first edge, Change data on the following    \
              edge */
#define SPI_CFG_CPHA_SECOND (1 << 4) /** SPI Clock Polarity Select */
#define SPI_CFG_CPOL_LO                                                        \
  (0 << 5) /** The rest state of the clock (between frames) is low. */
#define SPI_CFG_CPOL_HI                                                        \
  (1 << 5) /** The rest state of the clock (between frames) is high. */
...
#define SPI_CFG_SPOLNUM_HI(n)                                                  \
  (1 << ((n) + 8)) /** SSELN is active High, selects 0 - 3 */
...
#define SPI_DLY_FRAME_DELAY(n)                                                 \
  (((n) & 0x0F)                                                                \
   << 8) /** Minimum time in SPI clocks between adjacent data frames. */
...
#define SPI_STAT_TXUR                                                          \
  (1 << 3) /** Transmitter Underrun interrupt flag (In Slave Mode only) */
...
#define SPI_INTENSET_RXDYEN                                                    \
  (1 << 0) /** Enable Interrupt when receiver data is available */
#define SPI_INTENSET_TXDYEN                                                    \
  (1 << 1) /** Enable Interrupt when the transmitter holding register is       \
              available. */
#define SPI_INTENSET_RXOVEN                                                    \
  (1 << 2) /**  Enable Interrupt when a receiver overrun occurs */
#define SPI_INTENSET_TXUREN                                                    \
  (1 << 3) /**  Enable Interrupt when a transmitter underrun occurs (In Slave  \
              Mode Only)*/
#define SPI_INTENSET_SSAEN                                                     \
  (1 << 4) /**  Enable Interrupt when the Slave Select is asserted.*/
#define SPI_INTENSET_SSDEN                                                     \
  (1 << 5) /**  Enable Interrupt when the Slave Select is deasserted..*/

/**
 * Macro defines for SPI Interrupt Enable Clear register
 */
#define SPI_INTENCLR_BITMASK (0x3F) /** SPI INTENCLR Register BitMask */
#define SPI_INTENCLR_RXDYEN                                                    \
  (1 << 0) /** Disable Interrupt when receiver data is available */
#define SPI_INTENCLR_TXDYEN                                                    \
  (1 << 1) /** Disable Interrupt when the transmitter holding register is      \
              available. */
#define SPI_INTENCLR_RXOVEN                                                    \
  (1 << 2) /** Disable Interrupt when a receiver overrun occurs */
#define SPI_INTENCLR_TXUREN                                                    \
  (1 << 3) /** Disable Interrupt when a transmitter underrun occurs (In Slave  \
              Mode Only) */
#define SPI_INTENCLR_SSAEN                                                     \
  (1 << 4) /** Disable Interrupt when the Slave Select is asserted. */
#define SPI_INTENCLR_SSDEN                                                     \
  (1 << 5) /** Disable Interrupt when the Slave Select is deasserted.. */
...
#define SPI_RXDAT_RXSSELNUM_INACTIVE(n)                                        \
  (1 << ((n) + 16))             /** The state of SSELN pin is inactive */
...
#define SPI_TXDATCTL_CTRLMASK                                                  \
  (0xF710000) /** SPI TXDATCTL Register BitMask for control bits only */
#define SPI_TXDATCTL_ASSERT_SSEL (0 << 16)   /** Assert SSEL0 pin */
#define SPI_TXDATCTL_DEASSERT_SSEL (1 << 16) /** Deassert SSEL0 pin */
#define SPI_TXDATCTL_DEASSERTNUM_SSEL(n)                                       \
  (1 << ((n) + 16))                           /** Deassert SSELN pin */
...
#define SPI_TXCTL_DEASSERTNUM_SSEL(n)                                          \
  (1 << ((n) + 16))                           /** Deassert SSELN pin */
#define SPI_TXDATCTL_DEASSERT_ALL (0xF << 16) /** Deassert all SSEL pins */
 ...
 */
#define SPI_DIV_VAL(n)                                                         \
  ((n) & 0xFFFF) /** Rate divider value mask (In Master Mode only)*/
...
#define SPI_INTSTAT_TXUR                                                       \
  (1 << 3) /** Transmitter Underrun interrupt flag (In Slave Mode only) */
#define SPI_INTSTAT_SSA (1 << 4) /** Slave Select Assert */
#define SPI_INTSTAT_SSD (1 << 5) /** Slave Select Deassert */

/** @brief SPI Clock Mode*/
#define SPI_CLOCK_CPHA0_CPOL0                                                  \
  (SPI_CFG_CPOL_LO | SPI_CFG_CPHA_FIRST) /**< CPHA = 0, CPOL = 0 */
#define SPI_CLOCK_MODE0                                                        \
  SPI_CLOCK_CPHA0_CPOL0 /**< Alias for CPHA = 0, CPOL = 0 */
#define SPI_CLOCK_CPHA1_CPOL0                                                  \
  SPI_CFG_CPOL_LO | SPI_CFG_CPHA_SECOND /**< CPHA = 0, CPOL = 1 */
#define SPI_CLOCK_MODE1                                                        \
  SPI_CLOCK_CPHA1_CPOL0 /**< Alias for CPHA = 0, CPOL = 1 */
#define SPI_CLOCK_CPHA0_CPOL1                                                  \
  SPI_CFG_CPOL_HI | SPI_CFG_CPHA_FIRST /**< CPHA = 1, CPOL = 0 */
#define SPI_CLOCK_MODE2                                                        \
  SPI_CLOCK_CPHA0_CPOL1 /**< Alias for CPHA = 1, CPOL = 0 */
#define SPI_CLOCK_CPHA1_CPOL1                                                  \
  SPI_CFG_CPOL_HI | SPI_CFG_CPHA_SECOND /**< CPHA = 1, CPOL = 1 */
#define SPI_CLOCK_MODE3                                                        \
  SPI_CLOCK_CPHA1_CPOL1 /**< Alias for CPHA = 1, CPOL = 1 */

  /**
   * @brief	Set SPI CFG register values
   * @param	pSPI	: The base of SPI peripheral on the chip
   * @param	bits	: CFG register bits to set, amd OR'ed value of SPI_CFG_*
   * definitions
   * @return	Nothing
   * @note	This function safely sets only the selected bits in the SPI CFG
   * register. It can be used to enable multiple bits at once.
   */
   ...
   * @param	pSPI	: The base of SPI peripheral on the chip
   * @param	bits	: CFG register bits to clear, amd OR'ed value of
   * SPI_CFG_* definitions
   * @return	Nothing
   * @note	This function safely clears only the selected bits in the SPI
   * CFG register. It can be used to disable multiple bits at once.
   */
   ...
   * @note	Mask the return value with a value of type SPI_STAT_* to
   * determine if that status is active.
   */
   ...
   * @note	The return value is masked with 0xFFFF to not exceed 16-bits.
   * All other status bits are thrown away. This register should only be read if
   * it has data in it. This function is useful for systems that don't need SPI
   ...
   * @param	pSPI	: The base of SPI peripheral on the chip
   * @param	bits	: TXCTRL register bits to set, amd OR'ed value of
   * SPI_TXDATCTL_* definitions
   * @return	Nothing
   * @note	This function safely sets only the selected bits in the SPI
   * TXCTRL register. It can be used to enable multiple bits at once.
   */
   ...
   * @param	pSPI	: The base of SPI peripheral on the chip
   * @param	bits	: TXCTRL register bits to clear, amd OR'ed value of
   * SPI_TXDATCTL_* definitions
   * @return	Nothing
   * @note	This function safely clears only the selected bits in the SPI
   * TXCTRL register. It can be used to disable multiple bits at once.
   */
   ...
   * @return	Nothing
   * @note	Selectable control states include
   * SPI_TXCTL_DEASSERTNUM_SSEL(0/1/2/3), SPI_TXCTL_EOT, SPI_TXCTL_EOF,
   * SPI_TXCTL_RXIGNORE, and SPI_TXCTL_FLEN(bits).
   */
   ...
   * @param	pSPI	: The base of SPI peripheral on the chip
   * @param	numBits	: Number of bits to transmit and receive, must be 1 to
   * 16
   * @return	Nothing
   */
