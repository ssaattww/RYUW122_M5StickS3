...
#define WDT_OSC (LPC8XX_IRC_FREQ)
  ...
  typedef struct
  {                    /*!< WWDT Structure         */
    __IO uint32_t MOD; /*!< Watchdog mode register. This register contains the
                          basic mode and status of the Watchdog Timer. */
    __IO uint32_t TC;  /*!< Watchdog timer constant register. This register
                          determines the time-out value. */
    __O uint32_t FEED; /*!< Watchdog feed sequence register. Writing 0xAA
                          followed by 0x55 to this register reloads the Watchdog
                          timer with the value contained in WDTC. */
    __I uint32_t TV; /*!< Watchdog timer value register. This register reads out
                        the current value of the Watchdog timer. */
    __I uint32_t RESERVED0;
    __IO uint32_t
        WARNINT; /*!< Watchdog warning interrupt register. This register
                    contains the Watchdog warning interrupt compare value. */
    __IO uint32_t WINDOW; /*!< Watchdog timer window register. This register
...
#define WWDT_WDMOD_WDINT ((uint32_t)(1 << 3))
   ...
   * @param	pWWDT	: The base of WatchDog Timer peripheral on the chip
   * @param	timeout	: WDT timeout in ticks, between WWDT_TICKS_MIN and
   * WWDT_TICKS_MAX
   * @return	none
   */
   ...
   * @param	pWWDT	: The base of WatchDog Timer peripheral on the chip
   * @return	None
   * @note	If this function isn't called, a watchdog timer warning will
   * occur. After the warning, a timeout will occur if a feed has happened.
   */
   ...
   * @return	None
   * @note	This is the number of ticks after the watchdog interrupt that
   * the warning interrupt will be generated.
   */
   ...
   * WWDT_TICKS_MAX
   * @return	None
   * @note	The watchdog timer must be fed between the timeout from the
   * Chip_WWDT_SetTimeOut() function and this function, with this function
   * defining the last tick before the watchdog window interrupt occurs.
   */
   ...
   *						WWDT_WDMOD_WDEN,
   * WWDT_WDMOD_WDRESET, and WWDT_WDMOD_WDPROTECT
   * @return	None
   * @note	You can enable more than one option at once (ie,
   * WWDT_WDMOD_WDRESET | WWDT_WDMOD_WDPROTECT), but use the WWDT_WDMOD_WDEN
   * after all other options are set (or unset) with no other options. If
   * WWDT_WDMOD_LOCK is used, it cannot be unset.
   */
   ...
   * WWDT_WDMOD_WDRESET, and WWDT_WDMOD_WDPROTECT
   * @return	None
   * @note	You can disable more than one option at once (ie,
   * WWDT_WDMOD_WDRESET | WWDT_WDMOD_WDTOF).
   */
   ...
   * @param	pWWDT	: The base of WatchDog Timer peripheral on the chip
   * @param	status	: Or'ed value of status flag(s) that you want to clear,
   * should be:
