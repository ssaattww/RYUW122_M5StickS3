...
#define MRT_INTVAL_IVALUE                                                      \
  (0x7FFFFFFFUL) /* Maximum interval load value and mask */
#define MRT_INTVAL_LOAD                                                        \
  (0x80000000UL) /* Force immediate load of timer interval register bit */
...
#define MRTn_INTFLAG(ch) (1 << (ch))
   ...
   * @return	Nothing
   * @note	Setting bit 31 in timer time interval register causes the time
   * interval value to load immediately, otherwise the time interval value will
   * be loaded in next timer cycle.<br> Example: Chip_MRT_SetInterval(pMRT,
   * 0x500 | MRT_INTVAL_LOAD); // Will load timer interval immediately<br>
   * Example: Chip_MRT_SetInterval(pMRT, 0x500); // Will load timer interval
   * after internal expires
   */
