...
#define SYSCTL_CLOCK_ACMP SYSCTL_CLOCK_ACOMP
   ...
   * @param	div		: Watchdog divider value, even value between 2
   * and 64
   * @return	Nothing
   ...
   * @return	Nothing
   * @note	Use 0 to disable, or a divider value of 1 to 255. The system
   * clock rate is the main system clock divided by this value.
   */
  ...
  {
    LPC_SYSCTL->SYSAHBCLKCTRL = (1 << clk) | (LPC_SYSCTL->SYSAHBCLKCTRL &
   ...
   * @param	fEnable	: true to use fractional clocking, false for integer
   * clocking
   * @return	Actual rate generated
   * @note	USARTs 0 - 2 use the same base clock for their baud rate
   *			basis. This function is used to generate that clock,
   * while the UART driver's SetBaud functions will attempt to get the closest
   *			baud rate from this base clock without altering it. This
   * needs to be setup prior to individual UART setup.<br> UARTs need a base
   * clock 16x faster than the baud rate, so if you need a 115.2Kbps baud rate,
   * you will need a clock rate of at least (115.2K * 16). The UART base clock
   * is generated from the main system clock, so fractional clocking may be the
   * only possible choice when using a low main system clock frequency. Do not
   * alter the FRGCLKDIV register after this call.
   */
   ...
   */
  STATIC INLINE uint32_t Chip_Clock_GetMainOscRate(void) { return OscRateIn; }
   ...
   * @return	Nothing
   * @note	Use 0 to disable, or a divider value of 1 to 255. The CLKOUT
   * clock rate is the clock source divided by the divider. This function will
