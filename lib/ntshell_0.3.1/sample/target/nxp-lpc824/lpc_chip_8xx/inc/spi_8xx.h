...
extern "C"
{
  ...
  typedef struct
  {
    ...
    uint8_t FrameDelay; /* Delay value between frames of a transfer in SPI clock
                           time */
   ...
   * @return	Nothing
   */
  void Chip_SPI_Int_Cmd(LPC_SPI_T *pSPI, uint32_t IntMask,
  ...
  /**
   * @brief	Set control information including SSEL, EOT, EOF RXIGNORE and
   * FLEN
   * @param	pSPI	: The base of SPI peripheral on the chip
   * @param	Flen	: Data size (1-16)
   * @param	Flag	: Flag control (Or-ed values of SPI_TXCTL_*)
   * @note	The control information has no effect unless data is later
   * written to TXDAT
   * @return	Nothing
   */
  STATIC INLINE void Chip_SPI_SetControlInfo(LPC_SPI_T *pSPI, uint8_t Flen,
                                             uint32_t Flag)
  {
  ...
  STATIC INLINE void Chip_SPI_SendFirstFrame_RxIgnore(LPC_SPI_T *pSPI,
                                                      ...
                                                      uint8_t DataSize)
  {
    pSPI->TXDATCTL = SPI_TXDATCTL_ASSERT_SSEL | SPI_TXDATCTL_EOF |
                     SPI_TXDATCTL_RXIGNORE | SPI_TXDATCTL_FLEN(DataSize - 1) |
                     SPI_TXDATCTL_DATA(Data);
  }
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_SPI_SendFirstFrame(LPC_SPI_T *pSPI, uint16_t Data,
                                             uint8_t DataSize)
  {
    pSPI->TXDATCTL = SPI_TXDATCTL_ASSERT_SSEL | SPI_TXDATCTL_EOF |
  ...
  {
    pSPI->TXDATCTL = SPI_TXDATCTL_ASSERT_SSEL | SPI_TXDATCTL_EOF |
                     SPI_TXDATCTL_EOT | SPI_TXDATCTL_RXIGNORE |
                     SPI_TXDATCTL_FLEN(DataSize - 1) | SPI_TXDATCTL_DATA(Data);
  }
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_SPI_SendLastFrame(LPC_SPI_T *pSPI, uint16_t Data,
                                            uint8_t DataSize)
  {
    pSPI->TXDATCTL = SPI_TXDATCTL_ASSERT_SSEL | SPI_TXDATCTL_EOF |
                     SPI_TXDATCTL_EOT | SPI_TXDATCTL_FLEN(DataSize - 1) |
                     SPI_TXDATCTL_DATA(Data);
  }
   ...
   * @param	pSPI			: The base SPI peripheral on the chip
   * @param	xf_setup		: Pointer to a SPI_DATA_SETUP_T
   * structure that contains specified information about transmit/receive data
   * configuration
   ...
   * @param	pSPI			: The base SPI peripheral on the chip
   * @param	pXfSetup		: Pointer to a SPI_DATA_SETUP_T
   * structure that contains specified information about transmit/receive data
   * configuration
   * @return	Actual data length has been transferred
   * @note
   * This function can be used in both master and slave mode. It starts with
   * writing phase and after that, a reading phase is generated to read any data
   * available in RX_FIFO. All needed information is prepared through xf_setup
   * param.
   */
   ...
   * @return	Actual data length has been transferred
   * @note
   * This function can be used in both master and slave mode. First, a writing
   * operation will send the needed data. After that, a dummy reading operation
   * is generated to clear data buffer
   */
