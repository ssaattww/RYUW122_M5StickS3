...
extern "C"
{
   ...
   * This callback is called from the I2C slave handler when an I2C slave
   * address needs data to send. Return 0 to NACK the master and terminate the
   * transfer, or return a non-0 value with the value to send in *data.
   */
  typedef uint8_t (*I2CSlaveXferSend)(uint8_t *data);

  /** @brief I2C slave receive data callback
   * This callback is called from the I2C slave handler when an I2C slave
   * address has receive data. Return 0 to NACK the master and terminate the
   * transfer, or return a non-0 value to continue the transfer.
   */
  typedef uint8_t (*I2CSlaveXferRecv)(uint8_t data);

  /** @brief I2C slave service done callback
   * This callback is called from the I2C slave handler when an I2C slave
   * transfer is completed. It's used to indicate the end of a slave transfer.
   */
  typedef void (*I2CSlaveXferDone)(void);

  /**
   * Slave transfer are performed using 3 callbacks. These 3 callbacks handle
   * most I2C slave transfer cases. When the slave is setup and a slave
   * interrupt is receive and processed with the Chip_I2CS_XferHandler()
   * function in the I2C interrupt handler, one of these 3 callbacks is called.
   * The callbacks can be used for unsized transfers from the master.
   *
   * When an address is received, the SlaveXferAddr() callback is called with
   * the received address. Only addresses enabled in the slave controller will
   * be handled. The slave controller can support up to 4 slave addresses.
   *
   * If the master is going to perform a read operation, the SlaveXferSend()
   * callback is called. Place the data byte to send in *data and return a non-0
   * value to the caller, or return 0 to NACK the master. (Note the master
   * ACKS/NACKS to slave on reads, so this won't necessarily stop the slave
   * transfer.)<br>
   *
   * If the master performs a write operation, the SlaveXferRecv() callback is
   * called with the received data. Return a non-0 value to the caller, or
   * return 0 to NACK the master.<br>
   *
   * Once the transfer completes, the SlaveXferDone() callback will be
   * called.<br>
   */
  typedef struct
  {
    I2CSlaveXferStart slaveStart; /*!< Called when an matching I2C slave address
                                     is received */
    I2CSlaveXferSend
        slaveSend; /*!< Called when a byte is needed to send to master */
    I2CSlaveXferRecv
        slaveRecv; /*!< Called when a byte is received from master */
   ...
   * @return	Nothing
   * @note	Do not call this function until the slave interface is fully
   * configured.
   */
   ...
   * @param	pI2C	: Pointer to selected I2C peripheral
   * @return	Nothing
   * @note	This function sets the slave controller to continue
   * transmission. This should be called only when slave is pending. The
   * function writes a complete value to slave Control register, ORing is not
   * advised.
   */
   ...
   */
  STATIC INLINE void Chip_I2CS_SetSlaveAddr(LPC_I2C_T *pI2C, uint8_t slvNum,
                                            uint8_t slvAddr)
  {
  ...
  {
    pI2C->SLVADR[slvNum] =
        (pI2C->SLVADR[slvNum] & I2C_SLVADR_MASK) & ~I2C_SLVADR_SADISABLE;
  }
   ...
   * @param	pI2C	: Pointer to selected I2C peripheral
   * @param	extend	: true to extend I2C slave detect address 0 range, or
   * false to match to corresponding bits
   * @param	slvAddr	: Slave address qualifier, see SLVQUAL0 register in User
   * Manual
   * @return	Nothing
   * @note	Do not 'pre-shift' the slave address.
   */
  STATIC INLINE void Chip_I2CS_SetSlaveQual0(LPC_I2C_T *pI2C, bool extend,
                                             uint8_t slvNum)
  {
    slvNum = slvNum << 1;
    if ( extend )
   ...
   * @param	pI2C	: Pointer to selected I2C peripheral
   * @param	xfers	: Pointer to a I2CS_MULTI_XFER_T structure see notes
   * below
   * @return	Returns non-zero value on completion of transfer
   * @note	See @ref I2CS_XFER_T for more information on this function. When
   * using this function, the I2C_INTENSET_SLVPENDING and I2C_INTENSET_SLVDESEL
   * interrupts should be enabled and setup in the I2C interrupt handler to call
   * this function when they fire.
   */
