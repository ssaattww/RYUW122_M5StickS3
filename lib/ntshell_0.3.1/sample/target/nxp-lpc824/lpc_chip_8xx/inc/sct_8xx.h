...
#define CONFIG_SCT_nOU (6) /*!< Number of outputs */
  ...
  typedef struct
  {
    ...
    union
    {
      ...
      {
        __IO uint16_t
            REGMODE_L; /*!< match - capture registers mode register L */
        __IO uint16_t
            REGMODE_H; /*!< match - capture registers mode register H */
      };
    };

    __IO uint32_t OUTPUT; /*!< output register */
    __IO uint32_t
        OUTPUTDIRCTRL;     /*!< output counter direction Control Register */
    ...
    {
      __IO union
      { /*!< ...Match Reload / Capture Control value (offset 0x200) */
    ...
    __IO struct
    {                 /*!< EV[i].STATE / EV[i].CTRL (offset 0x300) */
    ...
    __IO struct
    {               /*!< OUT[i].SET / OUT[i].CLR  (offset 0x500) */
...
#define SCT_CONFIG_CLKMODE_INCLK                                               \
  (0x2 << 1) /*!< Input clock selected in CLKSEL field */
#define SCT_CONFIG_CLKMODE_INEDGECLK                                           \
  (0x3 << 1) /*!< Input clock edge selected in CLKSEL field */
 ...
 */
#define COUNTUP_TO_LIMIT_THEN_CLEAR_TO_ZERO                                    \
  0 /*!< Direction for low or unified counter */
...
#define SCT_CTRL_PRE_L(x)                                                      \
  (((x) & 0xFF) << 5) /*!< Prescale clock for low or unified counter */
...
#define SCT_RES_TOGGLE_OUTPUT (3)
   ...
   */
  void Chip_SCT_SetClrControl(LPC_SCT_T *pSCT, uint32_t value,
                              FunctionalState ena);
   ...
   *					        - SCT_RES_SET_OUTPUT	:Set
   * output
   *					        - SCT_RES_CLEAR_OUTPUT	:Clear
   * output
   *					        - SCT_RES_TOGGLE_OUTPUT :Toggle
   * output : SCT_RES_NOCHANGE : DISABLE - To clear the field specified by value
   * @return	Nothing
   * Set conflict resolution for the output \a outnum
   */
  void Chip_SCT_SetConflictResolution(LPC_SCT_T *pSCT, uint8_t outnum,
                                      uint8_t value);
  ...
  /**
   * @brief	Enable the interrupt for the specified event in State
   * Configurable Timer
   * @param	pSCT	: The base of SCT peripheral on the chip
  ...
  /**
   * @brief	Disable the interrupt for the specified event in State
   * Configurable Timer
   * @param	pSCT	: The base of SCT peripheral on the chip
   * @param	evt		: Event value
   * @return	Nothing
