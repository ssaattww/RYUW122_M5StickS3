...
#define CRC_MODE_WRDATA_BIT_RVS                                                \
  (1 << 2) /** Select Bit order reverse for WR_DATA (per byte) */
...
#define CRC_MODE_SUM_BIT_RVS                                                   \
  (1 << 4)                         /** Select Bit order reverse for CRC_SUM */
#define CRC_MODE_SUM_CMPL (1 << 5) /** Select One's complement for CRC_SUM */

#define MODE_CFG_CCITT                                                         \
  (0x00) /** Pre-defined mode word for default CCITT setup */
#define MODE_CFG_CRC16                                                         \
  (0x15) /** Pre-defined mode word for default CRC16 setup */
#define MODE_CFG_CRC32                                                         \
  (0x36) /** Pre-defined mode word for default CRC32 setup */
...
#define CRC_SEED_CRC32 (0xFFFFFFFF) /** Initial seed value for CRC32 mode */
   ...
   * @return	Nothing
   * @note	Flags for setting up the mode word include
   * CRC_MODE_WRDATA_BIT_RVS, CRC_MODE_WRDATA_CMPL, CRC_MODE_SUM_BIT_RVS, and
   * CRC_MODE_SUM_CMPL.
   */
  ...
  /**
   * @brief	Engage the CRC engine with defaults based on the polynomial to
   * be used
   * @param	poly	: The enumerated polynomial to be used
   * @return	Nothing
   */
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_CRC_SetMode(uint32_t mode) { LPC_CRC->MODE = mode; }

  /**
   * @brief	Get the CRC Mode bits
   * @return	The current value of the CRC Mode bits
   */
  STATIC INLINE uint32_t Chip_CRC_GetMode(void) { return LPC_CRC->MODE; }
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_CRC_SetSeed(uint32_t seed) { LPC_CRC->SEED = seed; }

  /**
   * @brief	Get the CRC seed value
   * @return	Seed value
   */
  STATIC INLINE uint32_t Chip_CRC_GetSeed(void) { return LPC_CRC->SEED; }
   ...
   * @return	Nothing
   */
  STATIC INLINE void Chip_CRC_Write8(uint8_t data) { LPC_CRC->WRDATA8 = data; }
  ...
  /**
   * @brief	Gets the CRC Sum based on the Mode and Seed as previously
   * configured
   * @return	CRC Checksum value
   */
  STATIC INLINE uint32_t Chip_CRC_Sum(void) { return LPC_CRC->SUM; }

  /**
   * @brief	Convenience function for computing a standard CCITT checksum
   * from an 8-bit data block
  ...
  /**
   * @brief	Convenience function for computing a standard CRC16 checksum
   * from 16-bit data block
  ...
  /**
   * @brief	Convenience function for computing a standard CRC32 checksum
