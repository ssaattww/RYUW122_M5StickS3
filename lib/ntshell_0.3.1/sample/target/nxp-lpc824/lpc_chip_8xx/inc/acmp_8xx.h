...
#define ACMP_CTRL_RESERVED                                                     \
  (7 | (1 << 5) | (1 << 7) | (0x3f << 14) | (1 << 22) | (1 << 24) |            \
   (0x1fu << 27))
#define ACMP_LAD_RESERVED (~0x7f)

#define ACMP_COMPSA_BIT (1 << 6) /* Comparator output control bit */
#define ACMP_COMPSTAT_BIT                                                      \
  (1 << 21) /* Comparator status, reflects the state of the comparator output  \
             */
...
#define ACMP_LADREF_MASK (0x1 << 6)
  ...
  typedef enum
  {
    ...
    ACMP_EDGESEL_BOTH =
        (2 << 3) /* Set the COMPEDGE bit on falling and rising edges */
  } ACMP_EDGESEL_T;

  /** Hysteresis selection for comparator */
  typedef enum
  {
    ACMP_HYS_NONE = (0 << 25), /* No hysteresis (the output will switch as the
                                  voltages cross) */
   ...
   * @param	pACMP	: Pointer to Analog Comparator block
   * @return	Status is an Or'ed value of ACMP_COMPSTAT_BIT or
   * ACMP_COMPEDGE_BIT
   */
  ...
  void Chip_ACMP_SetupAMCPRefs(LPC_CMP_T *pACMP, ACMP_EDGESEL_T edgeSel,
                               ACMP_POS_INPUT_T Posinput,
                               ACMP_NEG_INPUT_T Neginput, ACMP_HYS_T hys);

  /**
   * @brief	Sets up voltage ladder
   * @param	pACMP			: Pointer to Analog Comparator block
   * @param	ladsel			: Voltage ladder value (0 .. 31)
   * @param	ladrefVDDCMP	: Selects the reference voltage Vref for the
   * voltage ladder : false for VDD, true for VDDCMP pin
   * @return	Nothing
   */
  void Chip_ACMP_SetupVoltLadder(LPC_CMP_T *pACMP, uint32_t ladsel,
                                 bool ladrefVDDCMP);
