...
#define IAP_PARAM_ERROR                                                        \
  12 /*!< nsufficient number of parameters or invalid parameter */
...
#define IAP_CRP_ENABLED 19       /*!< Code read protection enabled */
   ...
   * @return	Status code to indicate the command is executed successfully or
   * not
   * @note	This command must be executed before executing "Copy RAM to
   * flash" or "Erase Sector" command. The end sector must be greater than or
   * equal to start sector number
   */
   ...
   * @param	dstAdd		: Destination flash address where data bytes are
   * to be written
   * @param	srcAdd		: Source flash address where data bytes are to
   * be read
   * @param	byteswrt	: Number of bytes to be written
   * @return	Status code to indicate the command is executed successfully or
   * not
   * @note	The addresses should be a 256 byte boundary and the number of
   * bytes should be 256 | 512 | 1024 | 4096
   */
   ...
   * @return	Status code to indicate the command is executed successfully or
   * not
   * @note	The end sector must be greater than or equal to start sector
   * number
   */
   ...
   * @param	strSector	: Start sector number
   * @param	endSector	: End sector number
   * @return	Offset of the first non blank word location if the status code
   * is SECTOR_NOT_BLANK
   * @note	The end sector must be greater than or equal to start sector
   * number
   */
   ...
   * @param	dstAdd		: Destination of the RAM address of data bytes
   * to be compared
   * @param	srcAdd		: Source of the RAM address of data bytes to be
   * compared
   * @param	bytescmp	: Number of bytes to be compared
   * @return	Offset of the first mismatch of the status code is COMPARE_ERROR
   * @note	The addresses should be a word boundary and number of bytes
   ...
   * @return	Status code to indicate the command is executed successfully or
   * not
   * @note	The page number must be greater than or equal to start page
   * number
   */
