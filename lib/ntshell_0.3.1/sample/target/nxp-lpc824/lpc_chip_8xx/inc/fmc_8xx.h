...
#define FMC_FMSSTOP_RESERVED 0x7ffe0000
  ...
  typedef enum
  {
    FLASHTIM_20MHZ_CPU = 0, /*!< Flash accesses use 1 CPU clocks. Use for up to
                               20 MHz CPU clock*/
    FLASHTIM_30MHZ_CPU = 1, /*!< Flash accesses use 2 CPU clocks. Use for up to
                               30 MHz CPU clock*/
  } FMC_FLASHTIM_T;
   ...
   * @return	Nothing
   * @note	For CPU speed up to 20MHz, use a value of 0. For up to 30MHz,
   * use a value of 1
   */
...
#define FMC_FLASHSIG_BUSY (1UL << 31)
   ...
   * @param	start	: Starting FLASH address for computation, must be
   * aligned on 16 byte boundary
   * @param	stop	: Ending FLASH address for computation, must be aligned
   * on 16 byte boundary
   * @return	Nothing
   * @note	Only bits 20..4 are used for the FLASH signature computation.
   *			Use the Chip_FMC_IsSignatureBusy() function to determine
   * when the signature computation operation is complete and the
   *			Chip_FMC_GetSignature() function to get the computed
   * signature.
   */
  ...
  /**
   * @brief	Start computation of a signature for a FLASH memory address and
   * block count
   * @param	start	: Starting FLASH address for computation, must be
   * aligned on 16 byte boundary
