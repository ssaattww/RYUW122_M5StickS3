...
extern "C"
{
  ...
  {
    __IO uint32_t
        CTRL; /*!< Offset: 0x000 Alarm/Wakeup Timer Control register */
    uint32_t Reserved[2];
    __IO uint32_t
        COUNT; /*!< Offset: 0x000C Alarm/Wakeup Timer Counter register */
  } LPC_WKT_T;
...
#define WKT_CTRL_CLEARCTR                                                      \
   ...
   * @return	Nothing
   * @note	This function should not be called if the WKT is already
   * counting.
   */
  ...
  /**
   * @brief	Start wake-up timer interrupt, set clock source, set timer
   * interval
   * @param	pWKT	: Pointer to WKT register block
