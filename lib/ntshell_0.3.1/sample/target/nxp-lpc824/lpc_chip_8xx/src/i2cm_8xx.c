 ...
 * @param	sclH	: Number of I2C_PCLK cycles for the SCL HIGH time value
 * between (2 - 9).
 * @param	sclL	: Number of I2C_PCLK cycles for the SCL LOW time value
 * between (2 - 9).
 * @return	Nothing
 * @note	The I2C clock divider should be set to the appropriate value
 * before calling this function The I2C baud is determined by the following
 * formula: <br> I2C_bitFrequency = (I2C_PCLK)/(I2C_CLKDIV * (sclH + sclL)) <br>
 *				where I2C_PCLK is the frequency of the System
 * clock and I2C_CLKDIV is I2C clock divider
 */
static void Chip_I2CM_SetDutyCycle(LPC_I2C_T *pI2C, uint16_t sclH,
                                   uint16_t sclL)
{
...
{
  uint32_t scl =
      Chip_Clock_GetSystemClockRate() / (Chip_I2C_GetClockDiv(pI2C) * busSpeed);
...
uint32_t Chip_I2CM_XferHandler(LPC_I2C_T *pI2C, I2CM_XFER_T *xfer)
{
  ...
  else if ( status & I2C_STAT_MSTPENDING )
  {
    ...
    case I2C_STAT_MSTCODE_TXREADY:
      ...
      {
        /* If receive queued after transmit then initiate master receive
         * transfer*/
        if ( xfer->rxSz )
        {
