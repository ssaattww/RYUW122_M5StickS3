...
STATIC void SPI_Send_Data_RxIgnore(LPC_SPI_T *pSPI, SPI_DATA_SETUP_T *pXfSetup)
{
  ...
  {
    Chip_SPI_SendLastFrame_RxIgnore(pSPI, pXfSetup->pTx[pXfSetup->TxCnt],
                                    pXfSetup->DataSize);
  }
  else
  {
...
STATIC void SPI_Send_Data(LPC_SPI_T *pSPI, SPI_DATA_SETUP_T *pXfSetup)
{
  if ( pXfSetup->TxCnt == (pXfSetup->Length - 1) )
  {
    Chip_SPI_SendLastFrame(pSPI, pXfSetup->pTx[pXfSetup->TxCnt],
                           pXfSetup->DataSize);
  }
  else
  {
...
STATIC void SPI_Send_Dummy(LPC_SPI_T *pSPI, SPI_DATA_SETUP_T *pXfSetup)
{
...
uint32_t Chip_SPI_CalClkRateDivider(LPC_SPI_T *pSPI, uint32_t bitRate)
{
  ...
  /* Get SPI clock rate */
  SPIClk = Chip_Clock_GetSystemClockRate(); /*The peripheral clock for both SPIs
                                               is the system clock*/
...
uint32_t Chip_SPI_RWFrames_Blocking(LPC_SPI_T *pSPI, SPI_DATA_SETUP_T *pXfSetup)
{
  ...
  Chip_SPI_ClearStatus(pSPI, SPI_STAT_CLR_RXOV | SPI_STAT_CLR_TXUR |
                                 SPI_STAT_CLR_SSA | SPI_STAT_CLR_SSD);
  Chip_SPI_SetControlInfo(pSPI, pXfSetup->DataSize,
                          SPI_TXCTL_ASSERT_SSEL | SPI_TXCTL_EOF);
